...
''' % (version, NODE_JS))
  ...
  def test_nostdincxx(self):
    ...
    for compiler in [EMCC, EMXX]:
      print(compiler)
      out, err = run_textmode_tuple([PYTHON, EMCC, path_from_root('tests', 'hello_world.cpp'), '-v'], stdout=PIPE, stderr=PIPE)
      out2, err2 = run_textmode_tuple([PYTHON, EMCC, path_from_root('tests', 'hello_world.cpp'), '-v', '-nostdinc++'], stdout=PIPE, stderr=PIPE)
      assert out == out2
      def focus(e):
