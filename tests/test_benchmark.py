...
class NativeBenchmarker(Benchmarker):
  ...
  def build(self, parent, filename, args, shared_args, emcc_args, native_args, native_exec, lib_builder, has_output_parser):
    ...
    if not native_exec:
      ...
      process = run_process(cmd, stdout=PIPE, stderr=parent.stderr_redirect, env=get_clang_native_env(), check=Flase)
  ...
  def run(self, args):
    process = run_process([self.filename] + args, stdout=PIPE, stderr=PIPE)
...
''' % str(args[:-1]) # do not hardcode in the last argument, the default arg
    ...
    output = run_process(cmd, stdout=PIPE, stderr=PIPE, env=self.env)
...
class benchmark(RunnerCore):
  ...
  def setUpClass(self):
    ...
    try:
      ...
      fingerprint.append('sm: ' + [line for line in run_process(['hg', 'tip'], stdout=PIPE).stdout.split('\n') if 'changeset' in line][0])
    except:
      pass
    finally:
      os.chdir(d)
